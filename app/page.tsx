  ...
  return (
    <>
      ...
      <div style={{ background: '#0A0A0A', color: '#E8E8E8', overflowX: 'hidden' }}>
        ...
        >
          {/* ── Dark cinematic background (always visible, ensures readability) */}
          <div style={{
            position: 'absolute', inset: 0, zIndex: 0,
            background: 'linear-gradient(165deg, #050400 0%, #0a0806 35%, #0d0b03 65%, #080808 100%)',
          }} />

          {/* ── Hero image — scale on scroll + parallax ─────────── */}
          <motion.div
            style={{ y: heroImgY, scale: heroImgScale, position: 'absolute', inset: '-12% 0', zIndex: 1, transformOrigin: 'center 40%' }}
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={HERO_IMG}
              alt="BŌRYKU overland vehicle build"
              onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none' }}
              style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 38%' }}
            />
          </motion.div>

          {/* ── Multi-layer dark gradients for depth ─────────────── */}
          <div style={{ position: 'absolute', inset: 0, zIndex: 2, background: 'linear-gradient(to bottom, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.22) 35%, rgba(0,0,0,0.55) 70%, #0A0A0A 100%)' }} />
          <div style={{ position: 'absolute', inset: 0, zIndex: 2, background: 'radial-gradient(ellipse 80% 55% at 50% 100%, rgba(255,85,31,0.13) 0%, transparent 65%)' }} />
          <div style={{ position: 'absolute', inset: 0, zIndex: 2, background: 'radial-gradient(ellipse 50% 60% at 0% 50%, rgba(0,0,0,0.4) 0%, transparent 70%)' }} />
          ...
          {/* ── Floating glassmorphism build card ────────────────── */}
          <motion.div
            ...
            animate={loaded ? {
              ...
              transition: {
                ...
                x: { delay: 3.8, duration: 0.8 },
                y: { delay: 4.6, duration: 4, repeat: Infinity, ease: 'easeInOut' },
              } as object,
            } : {}}
            style={{ position: 'absolute', bottom: '22%', right: 'max(40px, 7%)', zIndex: 5, display: 'none' }}
          >
            <GlassCard style={{ minWidth: 190 }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '8px', color: 'rgba(255,85,31,0.6)', letterSpacing: '0.15em', marginBottom: 8 }}>ACTIVE BUILD</div>
              <div style={{ fontFamily: 'var(--font-bebas)', fontSize: '20px', color: '#fff', letterSpacing: '0.06em' }}>4RUNNER BUILD</div>
              <div style={{ fontFamily: 'var(--font-rajdhani)', fontSize: '13px', color: 'rgba(255,255,255,0.42)', marginTop: 2 }}>12 items · $28,450</div>
              <div style={{ marginTop: 8, height: 2, background: 'rgba(255,255,255,0.06)', borderRadius: 1, overflow: 'hidden' }}>
                <div style={{ width: '72%', height: '100%', background: 'var(--orange)', borderRadius: 1 }} />
              </div>
            </GlassCard>
          </motion.div>
          ...
          {/* ── Hero text content ─────────────────────────────────── */}
          <motion.div
            style={{ y: heroTextY, opacity: heroOpacity, position: 'relative', zIndex: 5, padding: '0 clamp(20px, 5vw, 48px)', maxWidth: '1040px', width: '100%' }}
          >
            {/* Eyebrow */}
            <motion.div
              variants={fadeIn} initial="hidden" animate={loaded ? 'visible' : 'hidden'} custom={0}
              style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', letterSpacing: '0.24em', color: 'var(--orange)', textTransform: 'uppercase', marginBottom: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '14px' }}
            >
              <span style={{ display: 'inline-block', width: 32, height: 1, background: 'var(--orange)', opacity: 0.45 }} />
              RYKU BUILD PLATFORM  v1.0
              <span style={{ display: 'inline-block', width: 32, height: 1, background: 'var(--orange)', opacity: 0.45 }} />
            </motion.div>

            {/* Headline — cinematic word stagger */}
            <div style={{ overflow: 'hidden' }}>
              <motion.h1
                variants={fadeUp} initial="hidden" animate={loaded ? 'visible' : 'hidden'} custom={1}
                style={{
                  fontFamily: 'var(--font-bebas)',
                  fontSize: 'clamp(70px, 13.5vw, 168px)',
                  lineHeight: 0.86,
            ...
            <div style={{ overflow: 'hidden' }}>
              <motion.h1
                variants={fadeUp} initial="hidden" animate={loaded ? 'visible' : 'hidden'} custom={1.9}
                style={{
                  fontFamily: 'var(--font-bebas)',
                  fontSize: 'clamp(70px, 13.5vw, 168px)',
                  lineHeight: 0.86,
                  ...
                  color: 'var(--orange)',
                  margin: '0 0 28px',
                  textShadow: '0 4px 64px rgba(255,85,31,0.40)',
                }}
              >
                THE CHAOS
              </motion.h1>
            ...
            {/* Sub-headline */}
            <motion.p
              variants={fadeUp} initial="hidden" animate={loaded ? 'visible' : 'hidden'} custom={3}
              style={{
            ...
            >
              ...
              Select your vehicle. Choose your mission. Configure with precision.
            </motion.p>

            {/* CTAs */}
            <motion.div
              variants={fadeUp} initial="hidden" animate={loaded ? 'visible' : 'hidden'} custom={3.8}
              style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap', alignItems: 'center' }}
            >
              <PulseCTA href="/build" label="START YOUR BUILD" dataAction="hero-cta-primary" />
              <PulseCTA href="/builds" label="EXPLORE BUILDS" ghost dataAction="hero-cta-secondary" />
            </motion.div>
          </motion.div>

          {/* Scroll indicator */}
          <motion.div
            variants={fadeIn} initial="hidden" animate={loaded ? 'visible' : 'hidden'} custom={6}
            style={{ position: 'absolute', bottom: '28px', left: '50%', transform: 'translateX(-50%)', zIndex: 5, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}
          >
            <motion.div
              animate={{ y: [0, 9, 0] }}
              transition={{ repeat: Infinity, duration: 1.7, ease: 'easeInOut' }}
              style={{ width: 20, height: 32, border: '1.5px solid rgba(255,85,31,0.32)', borderRadius: 10, display: 'flex', justifyContent: 'center', paddingTop: 6 }}
            >
              <div style={{ width: 3, height: 7, background: 'var(--orange)', borderRadius: 2, opacity: 0.65 }} />
            </motion.div>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '0.22em', color: 'rgba(255,255,255,0.18)', textTransform: 'uppercase' }}>SCROLL</span>
          </motion.div>

          {/* Bottom fade into page */}
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '130px', background: 'linear-gradient(to bottom, transparent, #0A0A0A)', pointerEvents: 'none', zIndex: 5 }} />
        </section>
